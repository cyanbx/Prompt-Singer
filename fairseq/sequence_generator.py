...
class SequenceGenerator(nn.Module):
    ...
    def generate_decoder(
        ...
        if self.match_source_len: # False
            ...
            max_len = min(
                int(self.max_len_a * src_len + self.max_len_b), # 1.0 * src_len + 200
                self.max_len - 1, # 1024 - 1
            )
        assert (
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            if self.lm_model is not None and not aux_task_name: # False
            ...
            # handle max length constraint 若达到最大长度，则只选EOS
            if step >= max_len:
            ...
            if (
                ...
                if step < self.min_len:
                    # minimum length constraint (does not apply if using prefix_tokens) 最小长度时，不能选中eos
                    lprobs[:, self.eos] = -math.inf
            ...
            if self.should_set_src_lengths: # False
                self.search.set_src_lengths(src_lengths)

            if self.repeat_ngram_blocker is not None: # False
    ...
    def finalize_hypos(
        ...
        unfin_idx = bbsz_idx // beam_size # torch.div(bbsz_idx, beam_size, rounding_mode="trunc")
