...
@dataclass
class W2vBertConfig(FairseqDataclass):
...
class W2vBertModel(BaseFairseqModel):
    ...
    def forward(
        ...
        x_w2v2 = layer_results[self.w2v2_encoder_layers - 1][0]
        # TxBxC -> BxTxC
        x_w2v2 = x_w2v2.transpose(0, 1)
        ...
        x_w2v2 = x_w2v2[mask_indices].view(x_w2v2.size(0), -1, x_w2v2.size(-1))

        x_w2v2 = self.final_proj(x_w2v2)  # encoder_embed_dim -> final_dim

        x_w2v2 = self.compute_w2v2_preds(x_w2v2, y, negs)
        x = self.compute_mlm_preds(x)  # encoder_embed_dim -> num_classes

        # NOTE: Currently only taking the first index from the first var group as mlm targets.
        mlm_targets = q["targets"][:, :, 0]
